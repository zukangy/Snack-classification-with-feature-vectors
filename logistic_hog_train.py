...
if grid_search:
    ...
    with open('./best_hog_logic_params.yml', 'w') as file:
        yaml.dump(grid_search.best_params_, file, default_flow_style=False)
        
else:
    # Best params from grid search
    with open('./best_hog_logic_params.yml') as file:
    ...
    pca = PCA(**pca_params)
    logit = LogisticRegression(random_state=42, **model_params)
    ...
    plt.figure(figsize=(15, 12))  # You can adjust the size to fit your presentation needs
    sns.heatmap(df_conf_matrix, annot=True, fmt='d', cmap='Blues')
    plt.title('Logistic Regression on HOG Feature Vec Confusion Matrix Heatmap')
